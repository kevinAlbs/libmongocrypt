...
bool _mongocrypt_key_broker_add_doc(_mongocrypt_key_broker_t *kb,
    ...
    if (0 == ((int)kek_provider & kms_providers->configured_providers)) {
        ...
        CLIENT_ERR("client not configured with KMS provider necessary to decrypt: %s", key_doc->kek.kmsid);
        _key_broker_fail(kb);
        goto done;
    }
    ...
    if (kek_provider == MONGOCRYPT_KMS_PROVIDER_LOCAL) {
        ...
        if (kc.value.azure.access_token) {
            ...
            access_token = mc_mapof_kmsid_to_token_get_token(kb->crypt->cache_oauth, "azure");
        }
        if (!access_token) {

...
void _mongocrypt_opts_kms_providers_cleanup(_mongocrypt_opts_kms_providers_t *kms_providers) {
    ...
    for (size_t i = 0; i < kms_providers->named_mut.len; i++) {
        ...
        case MONGOCRYPT_KMS_PROVIDER_AWS: {
            bson_free(kcwid.creds.value.aws.secret_access_key);
            bson_free(kcwid.creds.value.aws.access_key_id);
            bson_free(kcwid.creds.value.aws.session_token);
            break;
        }
...
static bool kmsid_parse(const char *kmsid,
    ...
    *name_out = NULL;
    ...
    if (type_end == NULL) {
        ...
        BSON_ASSERT(diff >= 0 && (uint64_t)diff < SIZE_T_MAX);

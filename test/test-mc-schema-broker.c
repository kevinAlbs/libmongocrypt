...
static void test_mc_schema_broker_satisfy_from_cache(_mongocrypt_tester_t *tester) {
    ...
    // Ignores if no entry.
    {
        ...
        ASSERT(mc_scheme_broker_need_more_schemas(sb));
        ASSERT_OK_STATUS(mc_schema_broker_satisfy_from_cache(sb, &cache, status), status);
        ASSERT(mc_scheme_broker_need_more_schemas(sb)); // db.coll still not satisfied.
...
static void test_mc_schema_broker_insert_csfleEncryptionSchemas(_mongocrypt_tester_t *tester) {
    bson_t *schemaMap = TEST_FILE_AS_BSON("./test/data/schema-broker/schemaMap.json");
    ...
    // Does not append empty 'jsonSchema' for `bulkWrite`.
    {
        ...
        bson_t *cmd = TMP_BSON(BSON_STR({"bulkWrite" : 1, "nsInfo" : [ {"ns" : "db.coll"} ]}));
        ASSERT_OK_STATUS(mc_schema_broker_add_schemas_to_cmd(sb, cmd, MC_TO_MONGOCRYPTD, status), status);
...
static void test_mc_schema_broker_insert_encryptionInformation(_mongocrypt_tester_t *tester) {
    ...
    // Inserts empty encryptedFields when no collections have schemas and using the `bulkWrite` command.
    {
        mongocrypt_status_t *status = mongocrypt_status_new();
        mc_schema_broker_t *sb = mc_schema_broker_new();
    ...
    // Inserts empty QE schema in 'encryptionInformation' when one collection has a QE schema and other does not.
    {
        mongocrypt_status_t *status = mongocrypt_status_new();
        mc_schema_broker_t *sb = mc_schema_broker_new();
...
static void test_mc_schema_broker_satisfy_from_create_or_collMod(_mongocrypt_tester_t *tester) {
    ...
    // Ignores schema for an unrequested collection.
    {
        ...
        ASSERT(mc_scheme_broker_need_more_schemas(sb));
        ASSERT_OK_STATUS(mc_schema_broker_satisfy_from_create_or_collMod(sb, cmd, status), status);
        ASSERT(mc_scheme_broker_need_more_schemas(sb)); // Request for db.coll2 is still not satisfied.
        mc_schema_broker_destroy(sb);
        mongocrypt_status_destroy(status);
    }
...
void _mongocrypt_tester_install_mc_schema_broker(_mongocrypt_tester_t *tester) {
    ...
    INSTALL_TEST(test_mc_schema_broker_insert_csfleEncryptionSchemas);

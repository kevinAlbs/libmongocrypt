...
static void test_deterministic_contention(_mongocrypt_tester_t *tester) {
    ...
    mongocrypt_t *crypt = _mongocrypt_tester_mongocrypt(TESTER_MONGOCRYPT_SKIP_INIT);
    // TODO: configure random callback on `crypt`
    ASSERT_OK(mongocrypt_init(crypt), crypt);
    ...
    ASSERT_STATE_EQUAL(mongocrypt_ctx_state(ctx), MONGOCRYPT_CTX_READY);
    {
        ...
        ASSERT_CMPINT64(got_payload.contentionFactor, ==, 1); // TODO: set expected value.
...
void _mongocrypt_tester_install_ctx_encrypt(_mongocrypt_tester_t *tester) {
    ...
    INSTALL_TEST(test_deterministic_contention);
}
